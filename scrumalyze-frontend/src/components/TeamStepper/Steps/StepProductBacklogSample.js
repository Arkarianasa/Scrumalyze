...
const StepProductBacklogSample = ({ formValues, handleChange }) => {
  ...
  const secondaryScheme = prioritizationSchemes.find(
    (scheme) =>
      scheme.prioritizationSchemeID ===
      formValues.productBacklog.secondaryPrioritizationSchemeID
  );
    ...
    <Box>
      {formValues.backlogItems.map((item, index) => (
        <React.Fragment key={index}>
          <Box style={{ marginBottom: '20px', alignItems: 'center' }}>
            <Grid container spacing={2}>
              ...
              >
                <TextField
                  ...
                  value={item.itemName}
                  onChange={(e) =>
                    handleBacklogItemChange(index, 'itemName', e.target.value)
                  }
                  fullWidth
                  required
                />
                ...
                <Grid item xs={2}>
                  {primaryLevels.length > 0 ? (
                    <TextField
                      ...
                      onChange={(e) =>
                        handleBacklogItemChange(
                          index,
                          'primaryPriorityValue',
                          e.target.value
                        )
                      }
                      fullWidth
              ...
              {/* Secondary Prioritization Scheme */}
              {formValues.productBacklog.secondaryPrioritizationSchemeID !== null && (
                <Grid item xs={2}>
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      ...
                      onChange={(e) =>
                        handleBacklogItemChange(index, 'done', e.target.checked)
                      }
                      color="primary"
                    />
                  }
            ...
            <Grid container spacing={2} style={{ marginTop: '-5px' }}>
              <Grid item xs={12}>
                <TextField
                  ...
                  onChange={(e) =>
                    handleBacklogItemChange(index, 'itemDescription', e.target.value)
                  }
                  fullWidth
                  required
                />
          ...
          {index < formValues.backlogItems.length - 1 && (
